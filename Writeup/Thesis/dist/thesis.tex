...
\\
\>[B]{}\HSVar{gen\char95 sorted}\HSSym{\mathrel{=}}\HSVar{arbitrary}\HSSym{\bind} \HSVar{return}\ .\ \HSVar{diff}{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
...
\subsection{Isomorphisms}
  ...
  In this thesis, we will often talk about isomorphisms between types. An isomorphism between two types \agda{A} and \agda{B} asserts a one to one correspondence between their values. We use the following formal definition: 
...
\subsection{Generators for non-indexed types}
  ...
  Although our generator infrastructure is primarily designed for usage with indexed types, we can adapt it to work for non-indexed types. We do this by choosing a trivial index, and generating values of type \agda{⊤ → A} instead of generating values of the non-indexed type \agda{A} directly. With this approach, we can write a generator for natural numbers: 

\includeagda{2}{natgen}

  We will often take a more liberal approach to the notation of non-indexed generators, writing \agda{Gen A A} instead of \agda{Gen A (λ \{ tt → A\}) tt} and $\mu$ instead of $\mu$ \agda{tt}. 
...
\includeagdalisting{6}{signature}{Signatures}{lst:signatures}
  ...
  Contrary to the semantics of a W-type, which maps a code to a value in \agda{Set → Set}, 
...
\subsubsection{Vectors}
  ...
  as \agda{Vec a} (listing \ref{lst:defvec}). Indexed containers do not have an explicit 
...
\subsection{Restricting Operations and Arities}
  ...
  agian run into the problem that we have no way to generate values of type \agda{Op i} 
...
\includeagdanv{6}{sigfinreg}
    ...
    pattern match on the fixpoint of some code in \agda{Reg} instead of directly on the 
    operation or arity. 
...
\section*{Conclusion}
  ...
  enumeration derived using $\Sigma$-\agda{generate}. As was the case with the completeness 
...
\end{listing}
  ...
  pairs, mainly because both their semantics is a $\Sigma$-type which makes it 
  difficult to transfer their definitions directly to Haskell. 

\subsection{Generalized coproducts}

  In the original definition, the generalized coproduct is defined by a natural number 
  \ensuremath{\HSVar{n}} and a function that takes a value from the finite set of size \ensuremath{\HSVar{n}} (Fin n) to a 
  description number \ensuremath{\HSVar{n}}. We choose to use a \emph{size indexed list} or \emph{vector} 
  (or a size indexed list) in favor of a function, since this wil make it considerably 
...
\end{hscode}\resethooks
  ...
  We then require a singleton type for natural numbers to relate the first parameter 
...
\end{hscode}\resethooks
  ...
  in that vector. For example, we would map a vector \ensuremath{\HSVar{d1}\HSSym{:::}\HSVar{d2}\HSSym{:::}\HSCon{VNil}} to the type \ensuremath{\HSCon{Either}\;\HSSpecial{(}\HSCon{Interpret}\;\HSVar{d1}\HSSpecial{)}\;\HSSpecial{(}\HSCon{Interpret}\;\HSVar{d2}\HSSpecial{)}}. We build the appropriate type by induction 
  ...
  a coproduct of its interpretation and the empty type. A secondary reason is to 
  prevent derived generators from including large amounts of empty generators, which 
  often come with a serious performance penalty. 
...
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\HSSpecial{(}\HSSym{:+>\char126 }\HSSpecial{)}\HSSym{::}\HSCon{SNat2}\;\HSVar{n}\HSSym{\to} \HSCon{SVec}\;\HSVar{xs}\HSSym{\to} \HSCon{SingIDesc}\;\HSSpecial{(}\HSVar{n}\ {\color{gray}\mathbin{:\hspace{-0.1pt}\mathclap{\raisebox{0.1pt}{\scalebox{.9}{$+$}}}\hspace{-0.3pt}>}}\ \HSVar{xs}\HSSpecial{)}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\\
\>[9]{}\HSSym{\to} \HSSpecial{(}\HSVar{forall}\;\HSVar{s'}\ .\ \HSCon{Sing}\;\HSVar{s'}\HSSym{\to} \HSCon{Interpret}\;\HSVar{d}\HSSym{:\char126 :}\HSCon{Interpret}\;\HSSpecial{(}\HSCon{Expand}\;\HSVar{d}\;\HSVar{s'}\HSSpecial{)}\HSSpecial{)}{}\<[E]%
\\
...
\\
\>[B]{}\hsindent{5}{}\<[5]%
\>[5]{}\HSSpecial{(}\HSSym{:*:\char126 }\HSSpecial{)}\HSSym{::}\HSCon{SingIDesc}\;\HSVar{l}\HSSym{\to} \HSCon{SingIDesc}\;\HSVar{r}\HSSym{\to} \HSCon{SingIDesc}\;\HSSpecial{(}\HSVar{l}\ {\color{gray}\mathbin{:\hspace{-0.3pt}\mathclap{\raisebox{0.1pt}{\scalebox{1.2}{$*$}}}\hspace{-0.3pt}:}}\ \HSVar{r}\HSSpecial{)}{}\<[E]%
\\
\>[B]{}\hsindent{5}{}\<[5]%
\>[5]{}\HSSpecial{(}\HSSym{:+>\char126 }\HSSpecial{)}\HSSym{::}\HSCon{SNat2}\;\HSVar{n}\HSSym{\to} \HSCon{SVec}\;\HSVar{xs}\HSSym{\to} \HSCon{SingIDesc}\;\HSSpecial{(}\HSVar{n}\ {\color{gray}\mathbin{:\hspace{-0.1pt}\mathclap{\raisebox{0.1pt}{\scalebox{.9}{$+$}}}\hspace{-0.3pt}>}}\ \HSVar{xs}\HSSpecial{)}{}\<[E]%
\\
\>[B]{}\hsindent{5}{}\<[5]%
...
\\
\>[5]{}\hsindent{2}{}\<[7]%
\>[7]{}\HSSym{\to} \HSSpecial{(}\HSVar{forall}\;\HSVar{s'}\ .\ \HSCon{Sing}\;\HSVar{s'}\HSSym{\to} \HSCon{Interpret}\;\HSVar{d}\HSSym{:\char126 :}\HSCon{Interpret}\;\HSSpecial{(}\HSCon{Expand}\;\HSVar{d}\;\HSVar{s'}\HSSpecial{)}\HSSpecial{)}{}\<[E]%
\\
\>[5]{}\hsindent{2}{}\<[7]%
...
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\HSSym{<|>}\HSCon{Right}\ {\color{gray}\mathbin{<\hspace{-1.6pt}\mathclap{\raisebox{0.1pt}{\scalebox{.8}{\$}}}\hspace{-1.6pt}>}}\ \HSVar{idesc\char95 gen}\;\HSSpecial{(}\HSCon{SSuc2}\;\HSVar{n}\HSSym{:+>\char126 }\HSVar{ds}\HSSpecial{)}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\subsection{Well-typed lambda terms}
  ...
  well-formed involved. We use the description shown in listing \ref{lst:slcdescinductive} as a basis, based on the representation of well-typed terms 
  used in Phil Wadler and Wen Kokke's PLFA \cite{wadler2019plfa}.  
...
\end{hscode}\resethooks
  ...
  type occurs in the context. Now that we have all the necessary prerequisites in 
...
\>[3]{}\HSSym{:::\char126 }{}\<[3E]%
\>[9]{}\HSCon{SSigma}\;{}\<[17]%
\>[17]{}\HSSpecial{(}\HSCon{SVar}\;\HSSpecial{(}\HSSym{\lambda} \sigma \HSSym{\to} \HSSpecial{(}\Gamma \HSSpecial{,}\sigma \ {\color{gray}\mathbin{:\hspace{-0.1pt}\mathclap{\raisebox{0.1pt}{\scalebox{.9}{$-$}}}\hspace{-0.3pt}>}}\ \HSCon{T}\HSSpecial{)}\HSSpecial{)}\HSSym{:*:\char126 }\HSCon{SVar}\;\HSSpecial{(}\Gamma \HSSpecial{,}\HSSpecial{)}\HSSpecial{)}\;{}\<[E]%
\\
...
\end{hscode}\resethooks
  ...
  We now have everything needed in pace to start generating well-typed terms. We do 
