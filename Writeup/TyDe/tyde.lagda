...
\end{code}
  ...
  \section{Generation for Indexed Types}
    ...
    We extend the approach used for regular types to indexed types by applying the same techniques to two other type universes. First we consider \textit{indexed containers} (BRON), that allow us to describe a small subset of indexed types. Specifically, those types of which the indices of recursive subtrees of a value are uniquely determined by the index of the value itself. Subsequently we look at the universe of \textit{indexed descriptions} (BRON), which exteds the set of types we can describe by including a first-order combinator for sigma types. 

  \subsection{Indexed Containers}

    Indexed containers (BRON) describe datatypes as a Signature, which is a triple of \textit{operations}, \textit{arities} and \textit{typing}: 
...
\end{code}
  ...
  \subsubsection{Deriving generators for indexed containers}
    ...
    \item 
    Derive co-generators for regular types, e.g. generators producting values of type |R → b|, where R is regular. 
    \end{enumerate}
